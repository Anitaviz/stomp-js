...
function _setupListeners(stomp) {
    function _connected() {
        ...
        if (utils.really_defined(stomp["accept-version"])) {
...
function Stomp(args) {
    ...
    this['accept-version'] = args['accept-version'] || '1.0';
    this['heart-beat'] = args['heart-beat'] || '0,0';
...
Stomp.prototype.handle_new_frame = function(this_frame) {
    log.debug(this_frame.as_string())
    switch (this_frame.command) {
	    ...
	    log.debug('Protocol version: '+this_frame.headers['version'])
            this.emit('connected');
            break;
        case "RECEIPT":
...
//
// **Nacks a received message**
//
// Takes a string representing the message id to ack
//
Stomp.prototype.nack = function(message_id) {
    send_command(this, 'NACK', {'id': message_id});
    this.log.debug('Did not aknowledge message: ' + message_id);
};
